...
__global__ void compute_next_generation(int *d_grid, int *d_new_grid, int *d_alive_count, int *d_consecutive_alive_count, int N, int gen) {
	...
	int live_neighbors = 0;
		...
		//printf("_Cell_num-%d,%d,%d-_\n", idx, x, y);
			...
			grid.sync();
            // Swap pointers: make d_new_grid the current grid for the next iteration
...
int main(int argc, char *argv[]) {
	...
	// assign Block Grid dimensions
    dim3 blockDim(blockDimX, blockDimY);
    dim3 gridDim((N + blockDim.x - 1) / blockDim.x, (N + blockDim.y - 1) / blockDim.y);

	printf("gridDim:= %d x %d",(N + blockDim.x - 1) / blockDim.x  ,(N + blockDim.y - 1) / blockDim.y );
	printf("\n");
	
	// Kernel arguments
    void *args[] = { &d_grid, &d_new_grid, &d_alive_count, &d_consecutive_alive_count, &N, &generations };
    ...
    gpuErrchk(cudaLaunchCooperativeKernel((void*)compute_next_generation, gridDim, blockDim, args, 0, 0));
    ...
    printf("Time spent on (Single One Kernel) computation of %d x %d grid: %f ms\n", N, N, milliseconds);
	...
	// If verbose flag is set, write the result 
    if (verbose) {
		// Output file
		FILE *output_file = fopen("result_Single_Okern.txt", "w");
		if (output_file == NULL) {
			perror("Error opening result_Single_Okern.txt");
			return 1;
		}
	...
	save_results("data_Single_Okern.txt", N, milliseconds, blockDimX, gridDim.x, generations);
